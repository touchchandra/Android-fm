...
public class MediaService
    ...
    public static final String ACTION_QUIT = TAG + ".ACTION_QUIT";

    public static final String ACTION_PREV = TAG + ".ACTION_PREV";
    ...
    private ArrayList<FM> mPlaylist;
    private FM mCurrent;
    ...
    public static Intent createSinglePlayIntent(Context context, FM item) {
        ArrayList<FM> playlist = new ArrayList<FM>(1);
        playlist.add(item);
        return createPlaylistIntent(context, playlist);
    }

    public static Intent createPlaylistIntent(Context context, ArrayList<FM> playlist) {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (ACTION_START.equals(action)) {
            ArrayList<FM> playlist = intent.getParcelableArrayListExtra(EXTRA_PLAYLIST);
    ...
    private void onCommandStart(ArrayList<FM> playlist) {
    ...
    private void updateRemoteViews() {
        for (RemoteViews remoteViews : mRemoteViews) {
            ...
            remoteViews.setTextViewText(R.id.track_title,
                    (mCurrent != null ? mCurrent.getName() : getString(R.string.loading)));

            if (mCurrent != null) {
                if (mTrackImageUrl == null || !mTrackImageUrl.equals(mCurrent.getImageurl())) {
                    mTrackImageUrl = mCurrent.getImageurl();
    ...
    private void startPlaying(FM item) {
